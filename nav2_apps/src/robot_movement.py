...
class RobotMovement(Node):
    ...
    def get_transform(self, reference_frame, target_frame):
        ...
        try:
            ...
            if self.tf_buffer.can_transform(reference_frame, target_frame, rclpy.time.Time()):
                ...
                self.get_logger().warn(f"No transform available from {reference_frame} to {target_frame}")
    ...
    def obtain_transform(self, reference_frame, target_frame):
        # the method will obtain 1 transformation 
        transform_complete = False
        while transform_complete == False:
            transform = self.get_transform(reference_frame, target_frame)
            if transform != None:
